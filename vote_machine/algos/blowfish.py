...
class Blowfish:
    ...
    def decrypt(self, ct, IV = None):
        ...
        self.blocks.append(current_block)
        self.blocks.reverse()
        self.blocks[-1] = remove_trailig_zeros(self.blocks[-1])
        return "".join([from_bits(x) for x in self.blocks])

    def round_function(self, block, export_bits=True):
        parts = list(chunks(block, 8))
        val = (self.s_boxes[0][word_to_int(parts[0])] + self.s_boxes[1][word_to_int(parts[1])] % 2 ** 32)
        val = val ^ self.s_boxes[2][word_to_int(parts[2])]
        val = (val + self.s_boxes[3][word_to_int(parts[3])]) % 2 ** 32
        return int_to_bits(val) if export_bits else val
    ...
    def decrypt_block(self, block):
        ...
        for i in range(16, 0, -2):
            right_block = xor(int_to_bits(self.subkeys[i]), xor(self.round_function(left_block), right_block))
            left_block = xor(int_to_bits(self.subkeys[i - 1]), xor(self.round_function(right_block), left_block))
    ...
    def encrypt_block(self, block):
        ...
        for i in range(0, 16):
            ...
            right_block = xor(self.round_function(left_block), right_block)
...
s0 = [0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7, 0xb8e1afed, 0x6a267e96, 0xba7c9045, 0xf12c7f99, 0x24a19947,
...
s1 = [0x4b7a70e9, 0xb5b32944, 0xdb75092e, 0xc4192623, 0xad6ea6b0, 0x49a7df7d,
...
s2 = [0xe93d5a68, 0x948140f7, 0xf64c261c, 0x94692934, 0x411520f7, 0x7602d4f7,
...
s3 = [0x3a39ce37, 0xd3faf5cf, 0xabc27737, 0x5ac52d1b, 0x5cb0679e, 0x4fa33742,
